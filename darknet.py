...
def draw_boxes(detections, image, colors):
    ...
    for label, confidence, bbox in detections:
        ...
        left = int((left) / 224 * 512)
        right = int((right) / 224 * 512)
        top = int((top) / 224 * 256)
        bottom = int((bottom) / 224 * 256)
...
if os.name == "nt":
    ...
    lib = CDLL(os.path.join(os.environ.get(
        'DARKNET_PATH', './'), "libdarknet.so"), RTLD_GLOBAL)
...
network_predict_batch.argtypes = [
    c_void_p, IMAGE, c_int, c_int, c_int, c_float, c_float, POINTER(c_int), c_int, c_int]
