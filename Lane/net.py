...
from Lane.processing_image import warp_image
from Lane.hourglass_network import lane_network
...
class Net(object):
    ...
    def __init__(self):
        self.colours = np.array([[78, 142, 255], [204, 237, 221], [
                                92, 252, 255], [92, 255, 195], [159, 150, 255]])
    ...
    def load_model(self, epoch, loss_value):
        self.model.load_state_dict(torch.load(p.model_path+str(epoch)+'_'+str(
            loss_value)+'_'+'lane_detection_network.pkl', map_location='cuda:0'), False)
    ...
    def get_mask_lane(self, fits):
        ...
        for i, fit in enumerate(fits[:-1]):
            ...
            pts_right = np.array(
                [np.flipud(np.transpose(np.vstack([x_1, y])))])

            color = self.colours[i]

            pts = np.hstack((pts_left, pts_right))
            cv2.fillPoly(warp, np.int_([pts]), (int(
                color[0]), int(color[1]), int(color[2])))

        self.mask = cv2.warpPerspective(
            warp, p.inverse_perspective_transform, (warp.shape[1], warp.shape[0]))
        return self.mask
