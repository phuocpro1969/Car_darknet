...
def visualize_points_origin_size(x, y, test_image, ratio_w, ratio_h):
    ...
    for i, j in zip(x, y):
        ...
        for index in range(len(i)):
            cv2.circle(image, (int(i[index]), int(
                j[index])), 10, p.color[color], -1)
    cv2.imshow("test2", image)
    cv2.waitKey(0)
    ...
    for i in gt:
        for j in range(p.regression_size):  # gt
            ...
            if i[j] > 0:
                ...
                image = cv2.circle(
                    image, (x_value, y_value), 5, p.color[1], -1)
    cv2.imshow("image", image)
    cv2.waitKey(0)
...
def draw_points(x, y, image):
    ...
    for i, j in zip(x, y):
        ...
        if color_index > 12:
            color_index = 12
        for index in range(len(i)):
            image = cv2.circle(image, (int(i[index]), int(
                j[index])), 5, p.color[color_index], -1)
...
def get_steer_angle(fits):
    ...
    if len_fit >= 2:
        ...
        temp_y = 200
        temp_x = (np.poly1d(fits[-1])(temp_y) +
                  np.poly1d(fits[-2])(temp_y)) // 2
