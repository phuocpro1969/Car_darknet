    ...
    def __init__(self, in_channels, n_filters, k_size, padding, stride, bias=True, acti=True, dilation=1):
        ...
        if acti:
            ...
            self.cbr_unit = nn.Conv2d(
                in_channels, n_filters, k_size, padding=padding, stride=stride, bias=bias, dilation=dilation)
...
class bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, acti=True):
        ...
        self.conv2 = Conv2D_BatchNorm_Relu(
            temp_channels, temp_channels, 3, 1, 1)
        self.conv3 = Conv2D_BatchNorm_Relu(
            temp_channels, out_channels, 1, 0, 1, acti=self.acti)

        self.residual = Conv2D_BatchNorm_Relu(
...
class bottleneck_down(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = Conv2D_BatchNorm_Relu(
            temp_channels, temp_channels, 3, 1, 1, dilation=1)
        #self.conv3 = Conv2D_BatchNorm_Relu(temp_channels, out_channels, 1, 0, 1)
        self.conv3 = nn.Conv2d(temp_channels, out_channels,
...
class Output(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        self.conv1 = Conv2D_BatchNorm_Relu(
            in_size, in_size//2, 3, 1, 1, dilation=1)
        self.conv2 = Conv2D_BatchNorm_Relu(
            in_size//2, in_size//4, 3, 1, 1, dilation=1)
        self.conv3 = Conv2D_BatchNorm_Relu(
            in_size//4, out_size, 1, 0, 1, acti=False)
...
class hourglass_same(nn.Module):
    ...
    def forward(self, inputs):
        ...
        outputs = self.up1(self.prelu(
            self.bn1(outputs + self.residual1(outputs3, True))))
        outputs = self.up2(self.prelu(
            self.bn2(outputs + self.residual2(outputs2, True))))
        outputs = self.up3(self.prelu(
            self.bn3(outputs + self.residual3(outputs1, True))))
        outputs = self.up4(self.prelu(
            self.bn4(outputs + self.residual4(inputs, True))))
...
class my_resize_layer(nn.Module):
    def __init__(self, in_channels, out_channels, acti=True):
        super(my_resize_layer, self).__init__()
        self.conv1 = Conv2D_BatchNorm_Relu(
            in_channels, out_channels//4, 3, 1, 1, dilation=1, acti=False)
        self.conv2 = Conv2D_BatchNorm_Relu(
...
class hourglass_block(nn.Module):
    def __init__(self, in_channels, out_channels, acti=True, input_re=True):
        ...
        self.re2 = nn.Conv2d(out_channels, out_channels, 1,
                             padding=0, stride=1, bias=True, dilation=1)
        self.re3 = nn.Conv2d(1, out_channels, 1, padding=0,
                             stride=1, bias=True, dilation=1)
